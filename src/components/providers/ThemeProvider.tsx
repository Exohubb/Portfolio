...
const TEXT_SCALE: Record<TextSize, string> = { sm: '0.9', md: '1', lg: '1.15' }
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  const setTextSize     = (s: TextSize) => { setTextSizeState(s);      localStorage.setItem('textSize',      s)            }
  const setReducedMotion= (v: boolean)  => { setReducedMotionState(v); localStorage.setItem('reducedMotion', String(v))    }
  const setHighContrast = (v: boolean)  => { setHighContrastState(v);  localStorage.setItem('highContrast',  String(v))    }
